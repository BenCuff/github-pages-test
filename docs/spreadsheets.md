...
#### Provide workbooks in an ODS file format
...
#### Do not have any floating text boxes
...
#### Do not use blank rows or columns for separation
...
#### Do not have “freeze panes” enabled
...
#### Have only 1 header row, with a title in every cell, in bold font
...
#### Do not have split or merged cells, blank rows or columns, or hidden rows or columns
...
#### Present tables in long format in preference to wide format
...
#### Present rows in a logical order
...
#### Do not have filter options enabled
