...
Data is missing when no value is provided for a given variable. This is common for many datasets including surveillance datasets captured by the UKHSA. Missing values can reduce statistical power and introduce bias if not handled appropriately.

## Before analysis
You should try to understand the amount, causes and type of missingness as soon as you receive your dataset. This knowledge will help inform how you handle the data, analyse it and communicate your results. The earlier you recognise an issue, the quicker you can develop your plan to address it.
...
This could be as simple as counting or visualising the missing values in your dataset to understand their volume and location. Most software records missing data as a blank value or as ‘NA’ (not available), but check with your data providers in case they use special symbols. Pay particular attention to unexpected patterns, which may expose new underlying data quality issues.

Input validation tests (‘assertions’) are also encouraged early in your data analysis pipelines so you can 'sense check' your data. The [UKHSA’s Reproducible Analytical Pipeline (RAP) standards](rap-framework) recommend these tests should be automated when you need to re-run the same checks frequently, which will be particularly efficient for routine data collections.
...
The presence of missing data may indicate problems with the way your dataset is collected or processed. Understanding the causes can help you to fix the underlying problem and allow you to be more transparent with users of your data.
...
- data providers could not or did not provide data in time used in producing statistics for the next publication.
...
When there’s a pattern to the missingness that can be explained by another variable. For example, you might have missing blood-sample data that can be explained by men being less likely than women to submit a sample. A better term is perhaps ‘missing conditionally at random’.
{% endcapture %}
...
## Handle missing data
According to the [Code of Practice for Statistics](https://code.statisticsauthority.gov.uk/the-code/), producers of statistical publications are required to 'identify and address' any 'limitations in data sources'. As such, you must decide how to handle missing data in order to maintain reliability and transparency of your outputs.
...
It may be possible to go back to your data suppliers and recover data that was lost through administrative error, for example, or to request a re-collection of data for the cases where it’s missing. This may be difficult in practice because of the effort required, especially since you must ‘reduce the burden on those providing their information, and on those involved in collecting, recording and supplying data’ according to the [Code of Practice for Statistics](https://code.statisticsauthority.gov.uk/the-code/).
...
- complete cases, which means only the records that have a response value in every variable in the dataset (‘listwise deletion’)
...
1. Perform simple imputation (e.g. mean imputation) for all variables. These will be treated as ‘placeholders’ for now.
...
Incorporating variance into the replacement values can result in a better estimate of ‘true’ values and therefore a less-biased outcome compared to single imputation. The method is more complicated, however, given the need for selecting and specifying an appropriate imputation model.

## Communicate about missing data
Transparency is required when reporting on the prevalence and handling of missingness, statistical methods for imputation and the limitations of the approaches taken, although the level of detail should be proportionate to the complexity of methods used.
...
We recommend that you create a separate QMI document to provide extra detail that caters to the needs of different users and provides full transparency about how the statistics and data were produced.

Examples of specific information to include are:

- the volume of missing data by variable
- biases in the presence of missing data, like if prevalence was greater for records with certain characteristics
...
If the incomplete data is because of the data provider, explain at the start of the affected section the figures impacted and the plan to have the complete figures again from the soonest publication depending on the delay. At the end of the statistics, include a statement on your team's overall confidence in the statistics and briefly explain why.
...
You should represent missing data in data summaries like tables and charts so the user is aware that the data are incomplete. This will also ensure that missingness is acknowledged if the table or chart are used elsewhere in isolation and out of context.

Specifically for tables, you must use placeholder symbols that highlight when a value is missing. [Official guidance on symbols](https://analysisfunction.civilservice.gov.uk/policy-store/symbols-in-tables-definitions-and-help/) suggests using '[b]' for a break in time series, or '[u]' if the value is of low reliability, which could happen when summarising a variable that contains a lot of missing data. A placeholder symbol allows you to provide information about why a data point is missing, which is not apparent if you leave the cell blank. A blank cell may also lead to accessibility issues for users of assistive technology.

As an example, let's say that data wasn't collected for one particular year, perhaps due to the coronavirus pandemic. When you present this data in a table, you should add the placeholder symbol '[b]' to highlight the break to users, in this case for the year 2020.

Where data is missing, you should include a note under each affected table or chart to explain how it has been impacted by the missing data and explain why. Where that is not possible, use the previous table or chart and include a note under it and explain why. For example, when a chart relies on two data sources but the team has data from only one source.
...
{% include cards-container-start.html %}
  ...
  {% include card.html content=card_content_5 title="✔️ Best practice: use a placeholder symbol to indicate that data is missing." %}
{% include cards-container-end.html %}
...
{% include cards-container-start.html %}
  ...
  {% include card.html content=card_content_7 title="✔️ Best practice: use a placeholder symbol to indicate that data has been estimated." %}
{% include cards-container-end.html %}
...
# Make plot:
# Note: `theme_af()` is available from the SPD's data visualisation guidance page on Confluence:
df |>
